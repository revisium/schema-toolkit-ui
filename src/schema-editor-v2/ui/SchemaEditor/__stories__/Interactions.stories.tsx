...
const meta: Meta<typeof StoryWrapper> = {
  ...baseMeta,
  title: 'V2/SchemaEditor/Interactions',
};
export default meta;
...
export const ValidationErrorOnDuplicateField: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // Click "Review Errors" button (button text changes when there are errors)
    await waitFor(async () => {
      const reviewButton = canvas.getByTestId('schema-editor-create-button');
      await expect(reviewButton).toHaveTextContent('Review Errors');
    });
...
export const SwitchViewMode: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const schemaEditor = await canvas.findByTestId('schema-editor-v2');
...
export const AddArrayField: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // Note: Array fields have two type buttons - one for the field type and one for items type
    await waitFor(async () => {
