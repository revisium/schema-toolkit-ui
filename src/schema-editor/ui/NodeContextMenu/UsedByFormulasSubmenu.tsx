...
import type { FormulaDependent } from '../../model';

interface UsedByFormulasSubmenuProps {
  formulaDependents: FormulaDependent[];
}
      ...
      <Portal>
        <Menu.Positioner>
          <Menu.Content p="2" minW="240px">
            {formulaDependents.map((dep) => (
              <Box
                key={dep.formulaNodeId}
                py="1"
                borderBottomWidth={1}
              ...
              >
                ...
                <Box fontSize="xs" color="gray.500" fontFamily="mono">
                  {dep.expression}
                </Box>
              </Box>
            ))}
